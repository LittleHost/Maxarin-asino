...
async def fast_command(message: Message, state: FSMContext):
    """Создание быстрой игры (розыгрыша)"""
    args = message.text.split()
    if len(args) != 2:
        return await message.answer("❌ Использование: <code>/fast 10</code> - создать розыгрыш на 10 USDT", parse_mode=ParseMode.HTML)
    ...
    if amount < 0.1:
        return await message.answer("❌ Минимальная сумма розыгрыша — <b>0.1 USDT</b>")
...
async def show_mines_menu(message: Message, user_id: int, state: FSMContext, edit: bool = True):
    # Загружаем сохраненное количество мин из БД
    user_data = db.get_user_data(user_id)
