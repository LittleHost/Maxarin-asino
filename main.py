...
# --- Быстрые игры (Fast Game) ---
class FastGame:
    def __init__(self, game_id: int, creator_id: int, amount: float):
...
• <code>/fast [сумма]</code> - Создать розыгрыш (работает в чатах)
...
async def fast_command(message: Message, state: FSMContext):
    """Создание быстрой игры (розыгрыша) - работает ТОЛЬКО в чатах"""
    ...
    if len(args) != 2:
        return await message.answer("❌ Использование: <code>/fast 10</code> - создать розыгрыш на 10 USDT\n\n"
                                  "Примеры:\n"
                                  "<code>/fast 0.5</code> - розыгрыш на 0.5 USDT\n"
                                  "<code>/fast 1</code> - розыгрыш на 1 USDT\n"
                                  "<code>/fast 100</code> - розыгрыш на 100 USDT", 
    ...
    if amount < 0.01:
        return await message.answer("❌ Минимальная сумма розыгрыша — <b>0.01 USDT</b>")
    if amount > config.MAX_BET:
        return await message.answer(f"❌ Максимальная сумма розыгрыша — <b>{config.MAX_BET:.2f} USDT</b>")
    ...
    if not db.add_balance(user_id, -amount, is_bet=True):
    ...
    game = FastGame(game_id, user_id, amount)
...
async def fast_menu_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        ...
        "<code>/fast 0.5</code> - розыгрыш на 0.5 USDT\n"
        "<code>/fast 1</code> - розыгрыш на 1 USDT\n"
        "<code>/fast 10</code> - розыгрыш на 10 USDT\n\n"
        "📖 <b>Правила:</b>\n"
        "• Создатель указывает сумму (от 0.01 USDT)\n"
        "• Создатель автоматически становится участником\n"
        "• Еще 5 человек нажимают кнопку «Участвовать»\n"
...
async def show_fast_game(message: Message, game_id: int, state: FSMContext):
    """Отображение текущей быстрой игры"""
    ...
    text = f"🚀 <b>Розыгрыш №{game_id}</b>\n"
    text += f"💸 Сумма: <b>{game.amount:.2f} USDT</b>\n"
    ...
    text += f"\n💰 <b>Призовой фонд: {game.amount:.2f} USDT</b>"
    
    builder = InlineKeyboardBuilder()
    
    if not game.is_full():
        builder.row(InlineKeyboardButton(text="🎲 Участвовать", callback_data=f"fast_join:{game_id}"))
    
    builder.row(InlineKeyboardButton(text="❌ Отменить игру", callback_data=f"fast_cancel:{game_id}"))
...
async def fast_join_callback(callback: CallbackQuery, state: FSMContext):
    """Обработчик участия в быстрой игре"""
    ...
    if game.is_full():
        return await callback.answer("❌ Игра уже заполнена!", show_alert=True)
    ...
    if balance < game.amount:
        return await callback.answer(f"❌ Недостаточно средств! Нужно {game.amount:.2f} USDT", show_alert=True)
    
    if not db.add_balance(user_id, -game.amount, is_bet=True):
        return await callback.answer("❌ Ошибка при списании средств!", show_alert=True)
...
async def fast_cancel_callback(callback: CallbackQuery, state: FSMContext):
    """Отмена быстрой игры (только для создателя)"""
    game_id = int(callback.data.split(":")[1])
    game = fast_games.get(game_id)
    ...
    for player_id in game.players:
        db.add_balance(player_id, game.amount)
    
    del fast_games[game_id]
    await state.clear()
    
    await callback.message.edit_text(f"❌ Розыгрыш №{game_id} отменен. Средства возвращены участникам.", parse_mode=ParseMode.HTML)
...
async def start_fast_game(message: Message, game_id: int, state: FSMContext):
    ...
    text = f"🚀 <b>Розыгрыш №{game_id} начинается!</b>\n\n"
    text += f"<b>Участники:</b>\n{players_text}\n"
    text += f"💰 <b>Призовой фонд: {game.amount:.2f} USDT</b>\n\n"
    ...
    db.add_balance(winner_id, game.amount)
    
    winner_name = get_user_display_name(winner_id)
    
    final_text = f"🚀 <b>Розыгрыш №{game_id}</b>\n\n"
    final_text += f"<b>Участники:</b>\n{players_text}\n"
    final_text += f"🎲 <b>Выпало число: {dice_value}</b>\n\n"
    final_text += f"🏆 <b>ПОБЕДИТЕЛЬ:</b> {winner_name}\n"
    final_text += f"💰 <b>Выигрыш: {game.amount:.2f} USDT</b>"
    
    await message.answer(final_text, parse_mode=ParseMode.HTML)
    
    try:
        await message.bot.send_message(winner_id, f"🎉 Поздравляем! Вы выиграли {game.amount:.2f} USDT в розыгрыше №{game_id}!")
    except:
        pass
...
# --- СОЗДАНИЕ ЧЕКА (с защитой от nan) ---
