...
async def dice_text_handler(message: Message, state: FSMContext):
    ...
    text = re.sub(r"^(кубы|куб)\s*", "", text_raw).strip()
...
async def deposit_callback(callback: CallbackQuery, state: FSMContext):
    ...
    user_id = callback.from_user.id
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text=get_btn(user_id, "crypto_bot"), callback_data=f"deposit_cryptobot:{user_id}"))
    builder.row(InlineKeyboardButton(text=get_btn(user_id, "back"), callback_data=f"profile:{user_id}"))
    ...
    try:
        ...
        if win_coef > 0:
            ...
            text = (
                f"<b>👤 {user_name}</b>\n"
                f"<b>Побеждает в игре 🎲 на {bet:.2f} USDT</b>\n"
                f"<blockquote><b>× {win_coef} 🎄 Выигрыш {win_amount:.2f} USDT ❞</b></blockquote>\n\n"
                f"<b>📋 Баланс {new_balance:.2f} USDT</b>"
            )
            await state.update_data(processing_click=False)
...
# ==================== ОБРАБОТЧИК ДЛЯ ЭМОДЗИ-ИГР (dice_emoji, soccer, slots) ====================
