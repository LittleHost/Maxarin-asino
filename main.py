...
# ===== КЛАВИАТУРЫ (БЕЗ ТГП, ВОРЛД, РАКЕТА, СТАРС) =====
...
def get_round_order():
    """Порядок раундов для определения следующего"""
    return [
...
def format_bracket(matches, tournament_name):
    """Форматирование сетки с премиум эмодзи (без ТГП, Ворлд, Ракета, Старс)"""
...
def check_and_advance_round(tournament_name):
    """Проверяет, завершен ли текущий раунд, и создает следующий"""
    matches = get_tournament_matches(tournament_name)
    
    if not matches:
        return False, "Нет матчей"
    
    # Получаем все уникальные раунды в порядке
    rounds = []
    for match in matches:
        if match[5] not in rounds:  # round
            rounds.append(match[5])
    
    if not rounds:
        return False, "Нет раундов"
    
    # Берем последний (текущий) раунд
    current_round = rounds[-1]
    
    # Проверяем, все ли матчи в этом раунде завершены
    round_matches = [m for m in matches if m[5] == current_round]
    all_finished = all(m[7] == 1 for m in round_matches)  # is_finished
    ...
    for match in round_matches:
        score = match[6]  # score "3-2"
        if score and '-' in score:
            scores = score.split('-')
            try:
                if int(scores[0]) > int(scores[1]):
                    winners.append(match[3])  # player1
                else:
                    winners.append(match[4])  # player2
            except ValueError:
                winners.append(None)
        else:
            winners.append(None)
    ...
    # Сохраняем матчи следующего раунда
...
def update_bracket_message(tournament_name):
    """Обновляет сообщение с сеткой в чате"""
    matches = get_tournament_matches(tournament_name)
    tournament = get_tournament_by_name(tournament_name)
    ...
    message_id = tournament[6]  # message_id
...
async def cmd_turnir(message: types.Message):
    ...
    try:
        ...
        if advanced:
            # Обновляем сообщение с новыми матчами
            matches = get_tournament_matches(tournament_name)
            bracket_text = format_bracket(matches, tournament_name)
            if message_id:
                try:
